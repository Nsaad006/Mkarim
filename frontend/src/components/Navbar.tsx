...
import { Menu, X, ShoppingCart, Phone, Search, Sun, Moon, ChevronRight } from "lucide-react";
...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 transition-all duration-300">
      ...
      {/* Mobile Page Navbar - visible only on mobile below the header */}
      {/* Mobile Page Navbar - below header on all pages */}
      <div className="lg:hidden bg-background/80 backdrop-blur-xl border-b border-border">
        <div className="flex justify-center gap-1 px-3 py-1.5">
          {navLinks.map((link) => (
            <Link
              key={link.path}
              to={link.path}
              className={`flex-shrink-0 px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-[0.15em] transition-colors whitespace-nowrap ${
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-40 lg:hidden bg-black/50 backdrop-blur-2xl flex flex-col border-l border-white/10 shadow-[-20px_0_60px_rgba(0,0,0,0.6)] overflow-hidden"
          >
            ...
            <div className="flex-none flex items-center justify-between px-6 pt-4 pb-2 relative z-10 border-b border-white/10">
              <span className="text-[10px] font-black text-primary tracking-[0.3em] uppercase">Catégories</span>
              <Button
                variant="ghost"
                size="icon"
                aria-label="Fermer le menu"
                className="w-9 h-9 rounded-xl bg-white/10 text-white hover:bg-white/20 transition-colors"
            ...
            <div className="flex-1 overflow-y-auto relative z-10">
              ...
              <div className="px-4 py-3 border-b border-white/10">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-white/10 border border-white/10 rounded-lg pl-9 pr-3 py-2 text-[11px] font-medium text-white placeholder:text-white/30 focus:outline-none focus:border-primary/50 focus:bg-white/15 transition-all"
                  />
                  {categorySearch && (
                    <button
              ...
              {/* All categories - filtered by search */}
              <div className="divide-y divide-white/10 px-4">
                {categories
                  ...
                  >
                    <Link
                      ...
                      onClick={() => { setIsOpen(false); setCategorySearch(""); }}
                    >
                      <span className="text-base font-black text-white/90 group-hover:text-primary uppercase tracking-wide transition-colors">{cat.name}</span>
                ...
                {categories.filter(cat => !categorySearch || cat.name.toLowerCase().includes(categorySearch.toLowerCase())).length === 0 && (
                  <p className="text-white/40 text-xs text-center py-6 uppercase tracking-widest">Aucun résultat</p>
                )}
                {!categorySearch && (
                  <Link
                    to="/products"
                    ...
                    onClick={() => setIsOpen(false)}
                  >
                    <span className="text-sm font-black text-primary uppercase tracking-widest">Voir tout</span>
            ...
            <div className="flex-none px-6 pb-6 pt-3 border-t border-white/10 relative z-10">
              <div className="flex gap-3">
                ...
                <a href={`tel:${whatsappNumber.replace(/\s+/g, "")}`} className="flex-[3]">
                  <Button className="w-full bg-primary hover:bg-primary/90 text-white font-black uppercase tracking-widest h-10 rounded-xl text-xs px-4">
                    APPELER
                  </Button>
                </a>
              </div>
