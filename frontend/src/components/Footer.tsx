  ...
  const AccordionSection = ({ title, id, children }: { title: string, id: string, children: React.ReactNode }) => {
    ...
    return (
      <div className="border-b border-white/5 lg:border-none">
        ...
        >
          <h3 className="font-display font-bold text-white text-base uppercase tracking-widest">{title}</h3>
          {isOpen ? <ChevronUp className="w-4 h-4 text-primary" /> : <ChevronDown className="w-4 h-4 text-zinc-500" />}
        </button>
        <h3 className="hidden lg:block font-display font-bold text-white text-lg mb-8 uppercase tracking-widest">{title}</h3>
    ...
    <footer className="bg-zinc-950 border-t border-white/5 pt-16 pb-12">
      <div className="container-custom">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-0 lg:gap-12 mb-16">
          ...
          <div className="space-y-8 mb-12 lg:mb-0">
              ...
              <span className="font-display text-3xl font-black tracking-tighter">
                {storeName.split(" ").length > 1 ? (
                  <>
                    ...
                    <span className="text-white"> {storeName.split(" ").slice(1).join(" ")}</span>
                  </>
                ) : (
            ...
            <div className="flex gap-4 flex-wrap">
              {[
                ...
                <a key={i} href={link} target="_blank" rel="noopener noreferrer" className="w-12 h-12 rounded-xl bg-zinc-900 border border-white/5 flex items-center justify-center hover:bg-primary hover:text-white transition-all duration-300 group">
                  <Icon className="w-6 h-6 text-zinc-400 group-hover:text-white" />
                </a>
              ) : null)}
              {settings?.tiktokLink && (
                <a href={settings.tiktokLink} target="_blank" rel="noopener noreferrer" className="w-12 h-12 rounded-xl bg-zinc-900 border border-white/5 flex items-center justify-center hover:bg-primary hover:text-white transition-all duration-300 group">
                  <svg className="w-6 h-6 text-zinc-400 group-hover:text-white fill-current" viewBox="0 0 24 24">
              ...
              {categories.slice(0, 6).map((category) => (
                <li key={category.id}>
                  <Link
                    ...
                    className="text-zinc-400 hover:text-primary transition-colors text-base font-medium"
                  >
                    {category.name}
                  </Link>
                </li>
          ...
          <AccordionSection title="Contact" id="contact">
            <ul className="space-y-6">
              <li className="flex items-start gap-4 text-base text-zinc-400">
              ...
              <li className="flex items-center gap-4 text-base text-zinc-400">
                <div className="w-10 h-10 rounded-xl bg-zinc-900 border border-white/5 flex items-center justify-center flex-shrink-0">
        ...
        <div className="pt-12 border-t border-white/5 flex flex-col lg:flex-row justify-between items-center gap-8">
          ...
          <div className="flex flex-wrap justify-center items-center gap-6 text-[10px] md:text-xs font-bold uppercase tracking-[0.2em] text-zinc-500">
            {codEnabled && (
              <span className="flex items-center gap-3 bg-white/5 py-2 px-4 rounded-full">
